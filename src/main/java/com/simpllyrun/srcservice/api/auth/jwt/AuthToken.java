...
public class AuthToken {
    ...
    public AuthToken(Key key, String id) {
        this.key = key;
        this.token = createAuthToken(id);
    }

    public AuthToken(Key key, String id, String role) {
        this.key = key;
        this.token = createAuthToken(id, role);
    }
    ...
    private String createAuthToken(String id, String role) {
        Date now = new Date();
        Date expirationDate = new Date(now.getTime() + JWT_EXPIRATION);
        return Jwts.builder()
                .setSubject(id)
